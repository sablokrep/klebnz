...
pub fn addmatrix(
    ...
    windowsize: &str,
    threashold: &str,
) -> Result<Vec<(DenseMatrix<f64>, Vec<i32>)>, Box<dyn Error>> {
    ...
    for i in pathstructopen.iter() {
        if i[0] > threashold.parse::<f64>().unwrap() {
            classificationlabel.push(1);
        } else if i[0] < threashold.parse::<f64>().unwrap() {
            classificationlabel.push(0);
        }
    }

    let returntype: Vec<(DenseMatrix<f64>, Vec<i32>)> = vec![(finalrelease, classificationlabel)];
